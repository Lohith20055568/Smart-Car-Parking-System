...
# Instruct FastAPI to automatically encode any ObjectId into a string as a safety fallback
app = FastAPI(
...
@app.get('/')
def root():
    return {'message': 'Smart Car Parking Detection API is running', 'database_mode': 'memory_fallback' if using_memory() else 'mongodb'}

@app.get('/api/health')
def health():
    return {'status': 'ok', 'database': 'memory_fallback' if using_memory() else 'mongodb', 'time': datetime.utcnow().isoformat()}
...
def replace_slots(slots: List[Slot]):
    clean = [s.model_dump() for s in slots]
    return {'slots': upsert_slots(clean)}
...
async def detect_image(file: UploadFile = File(...)):
    ...
    # Persist the latest slot states so the dashboard and chart update after a
    # detection instead of continuing to show the original "unknown" values.
    ...
    return {'record': saved, 'result_url': f'/api/result/{os.path.basename(out_path)}'}
...
async def detect_video(file: UploadFile = File(...)):
    ...
    while frame_index < 90:
        ...
        if frame_index % 30 == 0:
            ...
            best_output = draw_results(frame, updated_slots, latest_detections_list)
        if latest_slots is not None:
            annotated_frame = draw_results(frame, latest_slots, latest_detections_list)
    ...
    avg_occ = round(sum(s['occupancy_rate'] for s in summaries) / len(summaries), 2) if summaries else 0
    record = {
        'source_type': 'video',
        'filename': file.filename,
